/**
 * 批量暂停推广脚本
 * 从 Excel 数据中筛选有成交但没花费的商品，批量暂停直通车推广
 */
const { launchBrowser, loadCookies, importCookies, closeBrowser } = require('../lib/browser');
const { parseGoodsIds, parsePromotionData, filterForPausePromotion, splitBatches } = require('../lib/excel-parser');

const BATCH_SIZE = 30;
const TARGET_URL = 'https://yingxiao.pinduoduo.com/goods/promotion/list';
...
async function execute({ goodsFile, reportFile, log, progress }) {
  ...
  try {
    ...
    const filteredIds = filterForPausePromotion(goodsIds, promotionDict);
    ...
    log(`共找到 ${filteredIds.length} 个需要暂停推广的商品。`);
    progress(5);
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        // 搜索商品
        const input = page.getByTestId('trTableSearch');
        await input.click();
        ...
        if (rows === 0) {
          log('本批次没有找到可推广的商品，跳过');
          skipped++;
          continue;
        }

        // 全选
        await page.locator('.anq-table-selection .anq-checkbox').click();
        await page.waitForTimeout(500);

        // 批量操作
        await page.locator('#batchOperationButton').click();
        await page.waitForTimeout(500);

        // 暂停
        const pauseBtn = page.locator('.anq-dropdown-menu-title-content:has-text("暂停")');
        await pauseBtn.click();
        await page.waitForTimeout(500);

        // 确认暂停
        await page.locator('button:has-text("确定暂停")').click();
        await page.waitForTimeout(3000);
